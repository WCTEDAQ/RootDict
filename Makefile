...
WCTE_RootDict.cxx: $(DATAMODEL)/DAQInfo.h $(DATAMODEL)/ReadoutWindow.h $(DATAMODEL)/MPMTMessages.h $(DATAMODEL)/TriggerType.h SerialisableObject.h WCTE_Linkdef.h
	rootcling $(SILENCERS) -f $@ -I. -I$(DATAMODEL) -I$(TOOLFDIR)/include -c $^

libWCTE_RootDict.so: WCTE_RootDict.cxx
	g++ $(SILENCERS) -std=c++11 -D__CLING__ -shared -fPIC -Wl,--no-undefined -o $@ $^ -I. -I$(DATAMODEL) -I`root-config --incdir` -I$(TOOLFDIR)/include `root-config --libs`
...
clean:
	rm -f libWCTE_RootDict.so  WCTE_RootDict.cxx  WCTE_RootDict_rdict.pcm
